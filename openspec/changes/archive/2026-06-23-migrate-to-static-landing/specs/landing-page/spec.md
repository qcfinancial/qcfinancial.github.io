...
The landing page SHALL be a single static `index.html` presenting the sections defined in the `README.md` design spec, in order: sticky nav, hero, trust strip, features (2×2), workflow (4 steps), markets (2 cards), install CTA, and footer. Content SHALL be centered with a max content width of 1200px and 32px horizontal padding.

#### Scenario: All sections render in order
- **WHEN** the page is loaded in a desktop browser
- **THEN** the nav, hero, trust strip, features, workflow, markets, install CTA, and footer appear in that vertical order with the design tokens (colors, typography, spacing, radii) from `README.md`
