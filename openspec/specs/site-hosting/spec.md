...
Build-less static hosting on GitHub Pages served from the `main` branch root, replacing the former MkDocs deployment pipeline.
...
Before obsolete files are deleted, the verbatim bilingual `T` translation strings and the `genMC()` generator from the original prototype SHALL be carried into the production source (`app.js`) or otherwise preserved, so no copy or generator logic is lost.
